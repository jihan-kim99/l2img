...
const TxtViewer = ({ fileText, narou, handleNextPage, subTitle }: { fileText: string, narou: boolean, handleNextPage: () => void, subTitle: string }) => {
      ...
      <Grid container justifyContent="center" alignItems="center">
        <Grid item xs={6} flex={1} flexWrap="wrap">
          {currentPageText.map((line, index) => (
            <Typography key={index} variant="body1" textAlign="start" paddingLeft={5} paddingTop={2}>
              {line}
            </Typography>
          ))}
        </Grid>
